(ns auto-core.i18n.missing-translation-report
...
(defn language-report
  ...
  (let [filtered-dictionary (select-keys dictionary expected-languages)]
    (apply merge-with
           union
           (map (fn [[language dict-map]] (into {} (map (fn [v] [v #{language}]) (keys dict-map))))
                filtered-dictionary))))
...
(defn key-with-missing-languages
  ...
  (let [key-set-exceptions (into #{} key-exceptions)]
    (filterv (fn [[k v]] (and (not (contains? key-set-exceptions k)) (not= v expected-languages)))
