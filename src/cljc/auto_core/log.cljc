...
(defmacro trace
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :trace)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log ~logger-id# :trace ~@message)
      `(auto-core.log.be-log/log ~logger-id# :trace ~@message))))
...
(defmacro trace-format
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :trace)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log-format ~logger-id# :trace ~fmt ~@args)
      `(auto-core.log.be-log/log-format ~logger-id# :trace ~fmt ~@args))))
...
(defmacro debug
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :debug)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log ~logger-id# :debug ~@message)
      `(auto-core.log.be-log/log ~logger-id# :debug ~@message))))
...
(defmacro debug-format
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :debug)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log-format ~logger-id# :debug ~fmt ~@args)
      `(auto-core.log.be-log/log-format ~logger-id# :debug ~fmt ~@args))))
...
(defmacro info
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :info)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log ~logger-id# :info ~@message)
      `(auto-core.log.be-log/log ~logger-id# :info ~@message))))
...
(defmacro info-format
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :info)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log-format ~logger-id# :info ~fmt ~@args)
      `(auto-core.log.be-log/log-format ~logger-id# :info ~fmt ~@args))))
...
(defmacro warn
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :warn)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log ~logger-id# :warn ~@message)
      `(auto-core.log.be-log/log ~logger-id# :warn ~@message))))
...
(defmacro warn-format
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :warn)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log-format ~logger-id# :warn ~fmt ~@args)
      `(auto-core.log.be-log/log-format ~logger-id# :warn ~fmt ~@args))))
...
(defmacro error
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :error)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log ~logger-id# :error ~@message)
      `(auto-core.log.be-log/log ~logger-id# :error ~@message))))
...
(defmacro error-format
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :error)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log-format ~logger-id# :error ~fmt ~@args)
      `(auto-core.log.be-log/log-format ~logger-id# :error ~fmt ~@args))))
...
(defmacro fatal
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :fatal)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log ~logger-id# :fatal ~@message)
      `(auto-core.log.be-log/log ~logger-id# :fatal ~@message))))
...
(defmacro fatal-format
  ...
  (let [logger-id# (auto-core.log.strategy/apply-strategy stgy *ns* :fatal)]
    (if (cljs-env? &env)
      `(auto-core.log.fe-log/log-format ~logger-id# :fatal ~fmt ~@args)
      `(auto-core.log.be-log/log-format ~logger-id# :fatal ~fmt ~@args))))
