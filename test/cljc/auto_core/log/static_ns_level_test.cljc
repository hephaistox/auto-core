...
(deftest apply-ns-rule-test
  (let [ns-chooser-stub (sut/make-static-ns-level-strategy sut/ns-rules)
    ...
    (testing "ns outside the scope are rejected"
      ...
      (is (nil? (sut/apply-ns-rule "auto-core" (get-stubbed-rule :ac-edn-utils)))))
...
(deftest choose-logger-test
  (let [ns-chooser-stub (sut/make-static-ns-level-strategy sut/ns-rules)
    ...
    (is (= [:auto-core.log.registry/text-based] (apply-strategy* "auto-core" :fatal))
    ...
    (is (= [:auto-core.log.registry/text-based] (apply-strategy* "non-existing-namespace" :fatal))
