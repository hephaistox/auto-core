...
(deftest log-test
  (testing
    ...
    (is (= (macroexpand `(core-log/trace "Hey"))
           (macroexpand `(sut/log [:auto-core.log.registry/no-op] :trace "Hey"))))
    (is (= `((juxt ~auto-core.log.be-registry/no-op-fn) ~*ns* :trace "Hey" "Ho")
           (macroexpand `(sut/log [:auto-core.log.registry/no-op] :trace "Hey" "Ho")))))
  ...
  (testing "Fatal is happening and printing the expected string"
    (is (= "Hey\n" (with-out-str (sut/log [:auto-core.log.registry/print] :fatal "Hey")))))
  (testing "Trace is happening and printing the expected string"
    (is (= "Hey\n" (with-out-str (sut/log [:auto-core.log.registry/print] :trace "Hey"))))))

(deftest fatal-exception-test
  (testing "Printed strategy applies to exception when fatal level is caught\n"
    (is (str/starts-with? (with-out-str (sut/log-exception [:auto-core.log.registry/print]
                                                           :fatal
